...
if(isset($_GET['t']))
{
    ...
    if($t=="1")
    {
        $sel1=mysqli_query($dbcon,"select * from student_point_temp where spid='$id'");
        $r1=mysqli_fetch_row($sel1);
        $ins=mysqli_query($dbcon,"INSERT INTO `student_point`(`rol_num`, `subcatid`, `level_id`, `mark`, `mark_typ`, `mark_id`, `docfile`, `year`) VALUES ('$r1[1]','$r1[2]','$r1[3]','$r1[4]','$r1[5]','$r1[6]','$r1[7]','$r1[8]')");
        if($ins>0)
        {
            $up=mysqli_query($dbcon,"update student_point_temp set st='1' where spid='$id'");
            if($up>0)
            {
                header("location:studnotification.php");
            }
        }
    }
    if($t=="2")
    {
        $up=mysqli_query($dbcon,"update student_point_temp set st='2' where spid='$id'");
        if($up>0)
        {
            header("location:studnotification.php");
        }
									...
									STUDENTS NOTIFICATION
                                                ...
                                                <div class="row">
                                                         ...
                                                         while($rcat=mysqli_fetch_row($sel_cat))
                                                        {
                                                           ?>
                                                            <tr>
                                                                <td colspan="8"><?php echo $rcat[1] ?></td>                                                                
                                                            </tr>
                                                            <?php
                                                            $selsub=mysqli_query($dbcon,"select * from subcategory_data where catid='$rcat[0]'");
                                                            if(mysqli_num_rows($selsub)>0)
                                                            {
                                                                
                                                                while($rsub=mysqli_fetch_row($selsub))
                                                                {
                                                                    $subid=$rsub[0];
                                                                    $data1=mysqli_query($dbcon,"select * from student_point_temp where subcatid='$subid' and st='0'");
                                                                    if(mysqli_num_rows($data1)>0)
                                                                    {
                                                                        $i=0;
                                                                        
                                                                        while($rdata1=mysqli_fetch_row($data1))
                                                                        {
                                                                            $i++;
                                                                            $x++;
                                                                            ?>
                                                            <tr>
                                                                <td style="widows: 100px;"><?php echo $i ?></td>
                                                                ...
                                                                <td>
                                                                    ...
                                                                    if($rdata1[5]=="1")
                                                                    {
                                                                        $ch1=mysqli_query($dbcon,"select * from subcat_mrk where id='$rdata1[6]'");
                                                                        $r1=mysqli_fetch_row($ch1);
                                                                        echo $r1[2];
                                                                    }
                                                                    else
                                                                    {
                                                                        $ch1=mysqli_query($dbcon,"select * from subcat_xmrk where id='$rdata1[6]'");
                                                                        $r1=mysqli_fetch_row($ch1);
                                                                        echo $r1[4];
                                                                    }
                                                                    ?>
                                                                ...
                                                                <td>
                                                                    <?php
                                                                    $selstud=mysqli_query($dbcon,"select * from student_data where rol_num='$rdata1[1]'");
                                                                    $rstud=mysqli_fetch_row($selstud);
                                                                    echo $rstud[5]
                                                                    ?>
                                                                </td>
                                                                <td>
                                                                    <span class="glyphicon glyphicon-certificate" data-toggle="modal" data-target="#myModal<?php echo $x ?>"></span>
                                                                          ...
                                                                          <div class="modal-content">
                                                                            <div class="modal-header">
                                                                              ...
                                                                              <div class="modal-body"><center>
                                                                                <p>
                                                                                    <img src="cert/<?php echo $rdata1[7] ?>" class="img img-responsive" />
                                                                                </p></center>
                                                                            </div>
                                                                ...
                                                                <td>
                                                                    <a href="studnotification.php?t=1&id=<?php echo $rdata1[0] ?>"><span class="label label-success"><span class="glyphicon glyphicon-thumbs-up"></span> Approve </span></a>
                                                                    <a href="studnotification.php?t=2&id=<?php echo $rdata1[0] ?>"><span class="label label-danger"><span class="glyphicon glyphicon-thumbs-down"></span> Reject </span></a>
                                                                </td>
                                                            </tr>
                                                            <?php
                                                                        }
